 ...
 * - $sb_jump_link_type: returns file or node, so that imce file links are output correctly as absolute links
 *
 */
?>
    ...
    <?php if ($sb_title) : ?>
      <h4 class="super-block__title">
          <?php if ($sb_jump_link_url) : ?>
            <?php if ($sb_jump_link_type = 'file') : ?>
              <?php print l(t($sb_title), $sb_jump_link_url, array('absolute' => true, 'html' => TRUE, 'attributes' => array('class' => array('super-block__title-link')))); ?>
            <?php else : ?>
        ...
        <?php if ($sb_image) : ?>
          <div class="super-block__image">
              <div class="super-block__image-wrapper">
                  <?php if ($sb_jump_link) : ?>
                    <?php if ($sb_jump_link_type = 'file') : ?>
                      <?php print l($sb_image, $sb_jump_link_url, array('absolute' => true, 'html' => TRUE)); ?>
                    <?php else : ?>
                  ...
                  <?php if (in_array('image', $sb_jump_link_location_array) && $sb_jump_link) : ?>
                    <div class="super-block__jump-link clearfix">
                        <?php if ($sb_jump_link_type = 'file') : ?>
                          <?php print l(t($sb_jump_link_title), $sb_jump_link_url, array('absolute' => true)); ?>
                        <?php else : ?>
