<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl" exclude-result-prefixes="php">
    ...
    <xsl:template name="hg_media">
        ...
        <xsl:for-each select="hg_media/item">
            ...
            <xsl:call-template name="general_element">
                ...
                <xsl:with-param name="element_format" select="string('base64')" />
                <xsl:with-param name="element_value" select="image_path" />
            </xsl:call-template>
    ...
    <xsl:template name="category">
        <xsl:text>s:8:"category";</xsl:text>
        <xsl:text>a:2:{</xsl:text>
        <xsl:call-template name="format_base64" />
        <xsl:text>s:5:"value";</xsl:text>
        <xsl:text>s:</xsl:text>
        <xsl:value-of select="string-length(php:functionString('base64_encode',category))"/>
        <xsl:text>:"</xsl:text>
        <xsl:value-of select="php:functionString('base64_encode',category)"/>
        <xsl:text>";</xsl:text>
        <xsl:text>}</xsl:text>
    </xsl:template>
