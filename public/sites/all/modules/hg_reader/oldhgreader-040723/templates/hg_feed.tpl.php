    ...
    <?php foreach ($feeds as $feed) : ?>
      <li class="hg-feed-item hg-feed-item-<?php print $feed['type'] == 'empty' ? 'empty' : $feed['nid']; ?> clearfix">
        ...
        <?php elseif ($feed['type'] == 'event') : ?>
          ...
          <?php if (is_array($fields['feed_fields_events']) && (count(array_intersect(array('Location', 'Location phone', 'Location URL', 'Location email'), $fields['feed_fields_events'])) > 0)) : ?>
            ...
            <?php print '</p>'; ?>
          <?php endif; ?>
          <?php if (is_array($fields['feed_fields_events']) && (in_array('Summary sentence', $fields['feed_fields_events']))) { print '<p class="hg-feed-event-summary-sentence">' . $feed['sentence'] . '</p>'; } ?>
          <?php if (is_array($fields['feed_fields_events']) && (in_array('Summary', $fields['feed_fields_events']))) { print '<div class="hg-feed-event-summary">' . $feed['summary'] . '</div>'; } ?>
          <?php if (is_array($fields['feed_fields_events']) && (count(array_intersect(array('Fee', 'Extras'), $fields['feed_fields_events'])) > 0)) : ?>
